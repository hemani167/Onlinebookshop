	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(part != null){
			try{
				...
				PreparedStatement pst = con.prepareStatement("insert into DummyBookDetails(email,id,booktype,authorname,bookname,price,quantity,front,back,description) values(?,?,?,?,?,?,?,?,?,?)");				pst.setString(1,email);
			    pst.setString(2,id);
		...
		response.sendRedirect("MainHome.jsp?flag=yes");
	}
